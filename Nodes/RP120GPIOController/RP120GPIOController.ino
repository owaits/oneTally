...
byte mac[6] = { 0x90, 0xA2, 0xDA, 0x41, 0xF3, 0x45 };
...
void beginTallyController()
{
  if(!tallyController)
  {
    //Initialise the tally pins as oiutputs.
...
void printWebTextInputIP(TClient& client, const char* name, const char* id, const IPAddress& ipAddress)
{
...
void printWebTextInputInt(TClient& client, const char* name, const char* id, int value)
{
  client.print(F("<label for='"));
  client.print(id);
  client.print(F("'>"));
  ...
  byte cameraSelectRead = 0;
    ...
    if(cameraSelect != cameraSelectRead || tallySelect != tallySelectRead)
    {
       ...
       tallySelect = tallySelectRead;
        ...
        if(tallyController)
        {
          digitalWrite(7, tallySelect == 6);
          digitalWrite(6, tallySelect == 5);
          digitalWrite(5, tallySelect == 4);
          digitalWrite(4, tallySelect == 3);
          digitalWrite(3, tallySelect == 2);
          digitalWrite(9, tallySelect == 1);
        }
    }
    else
    {
